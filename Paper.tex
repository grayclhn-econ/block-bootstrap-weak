...
requirement that it be $o(n)$, although their stronger assumption is
necessary for the bootstrap to be consistent under some forms of
heterogeneity.
...
\citepos{GoJ:03} when the expected block length is $o(\sqrt{n})$. When
the blocks are longer than $o(\sqrt{n})$, a case that \citet{GoW:02}
...
the Stationary Bootstrap the same relationship must hold for the
expected block length. If $\Delta_n = \Delta_0$ and $T_n = \pi n$ for
...
We can also use the same DGP to produce scenarios where
Theorem~\ref{T1}'s Assumption~2 is satisfied.  The first mimics a
setting where the break is estimated and the bootstrap is applied to
the post-break sample, but a small number of pre-break observations
are accidentally included in the data set as well. Here
$\Delta_n = \Delta_0$ and $T_n = T_0$ for some finite $T_0$. In this
case, Assumption 2 holds hold as long as $M_n = o(\sqrt{n})$.
Similarly, if the break is moderate with unknown timing in the sense
of \citet{ElM:07}, so $\Delta_n = \Delta / \sqrt{n}$ and
$T_n = \pi n$, then $D_n \approx \Delta c (1 - c)$ and Assumption 2
again holds as long as $M_n = o(\sqrt{n})$. Consequently, block sizes
of length $o(\sqrt{n})$ ensure that the conditions of
Theorem~\ref{T1}'s when there is a moderate amount of
...
block lengths can also satisfy Assumption 2.

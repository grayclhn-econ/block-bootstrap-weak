...
\begin{abstract}\noindent
  ...
  \ned\ process obeys a central limit theorem (CLT),
    established by \citet[\textit{Econometric Theory}]{Jon:97}.  In doing so, this paper
  ...
  bootstrapped partial sums satisfy a functional \clt\ (\fclt) under the same
  conditions.
...
proofs are presented after the main text of the paper; only proofs for
the stationary bootstrap are given, since proofs for the other methods
are similar and easier to construct.  All limits are taken as
$n \to \infty$ unless otherwise noted and $\lVert \cdot \rVert_r$
denotes the $L_r$-norm.
...
Note that De Jong does allow a little more flexibility in the
...
To prove~\eqref{eq:19}, it suffices (via the law of iterated expectations and Lemma~\ref{L2})
to show that
\begin{equation*}
  \sum_{j=1}^{J_n} \Em \big ( Z_{nj}^{*2} \ind \{ Z_{nj}^{*2} > \epsilon \} \big ) \to^p 0.
\end{equation*}
But note that
\begin{equation*}
  \sum_{j=1}^{J_n} \Em \big ( Z_{nj}^{*2} \ind \{ Z_{nj}^{*2} > \epsilon \} \big )
  \leq \max_{j=1,\dots,J_n} \Em \big ( \tfrac{ n }{ M_{nj} } Z_{nj}^{*2}
  \ind \{ \tfrac{ n }{ M_{nj} } Z_{nj}^{*2} >
  \min_{k=1,\dots,J_n} \tfrac{ n }{ M_{nk} } \epsilon \} \big )
\end{equation*}
almost surely. Lemmas~\ref{L1} and~\ref{L6} imply that the RHS converges to
...
\end{align*}
for large enough $n$. Lemma~\ref{L6} implies that
\begin{equation*}
...
\begin{multline*}
  ...
  \max_{d=1,\dots,D}
  \E[\sup_{\gamma \in [0,\delta]} (1/\delta) | W_n^{*}(\gamma + \gamma_d) - W_n^{*}(\gamma_d) |^2
  \times \\
...
implicit use of inequalities with explicit inequalities. Since the
proof follows De Jong's very closely, it is presented in a
supplementary online appendix.\footnote{%
  Supplementary material to this article is provided in
  ``Supplementary Appendix for `Block bootstrap consistency under weak
  assumptions,'{}'' which is available at Cambridge Journals Online
  (\url{journals.cambridge.org/ect}).}
...
See the supplementary online appendix for the proof of~\eqref{eq:34},
which follows \citet[Lemma 6.5]{Mcl:75b} and \citet[Lemma 3.5]{Mcl:77}
almost exactly and is also presented as Theorem~16.13 in
\citet{Dav:94}. The same comment that follows Lemma~\ref{L7} applies
here as well: the bounds apply equally well to $\Em$.

...
\footnote{Note that this is one reason that we are presenting our
  results using general triangular-array notation: to allow for the
  means to change over time for comparison with \citepos{GoW:02} and
  \citepos{GoJ:03} results.}
...
\footnote{Some of the details of the argument will typically need to
  ...
  argument.}%
\footnote{Lemmas~\ref{L3} and
  \ref{L5} are particularly
...
Lemma~\ref{L3}) and the following
...
Lemma~\ref{L5},
Equation~\eqref{eq:30}. Since $J_n$ and $M_{nj}$ are both $\Ms$-measureable
...
\end{align*}
The first equality follows from simple algerbra and the definition
...
Theorem~\ref{T1}'s assumptions ensure that the
original array obeys the \clt,~\eqref{eq:36} holds \citep[Theorem
2]{Jon:97}.  A similar argument applies to the asymptotic distribution
of $\sqrt{n}(\bar X_n^* - \bar X_n) / \hat\sigma_n^*$, completing the
proof.\qed }
...
\begin{lem}\label{L1}
  ...
  \ell_n^{1+\epsilon} \to^p 0$ as $n \to \infty$ for any positive
  $\epsilon$, and
  \item $\sum_{i=1}^{J_n} M_{ni}^2 = O_p(n/p_n)$.
  \end{enumerate}
\end{lem}
...
\begin{proof}[Proof of Lemma~\ref{L1}]
  ...
  substitutions.
  Using the same argument as in part 2 of this Lemma, it suffices to
  prove that
  $\sum\nolimits_{i=1}^{\lfloor C n p_n \rfloor} M_{ni}^2 =
  O_p(n/p_n)$ where $C$ is an arbitrary constant greater than 1.  The
  LLN implies that
  \begin{equation*}
    \tfrac{1}{C n p_n} \sum\nolimits_{i=1}^{\lfloor C n p_n \rfloor}
    M_{ni}^2 = \tfrac{2 - p_n}{p_n^2} + o_p(\tfrac{2 - p_n}{p_n^2})
  \end{equation*}
...
\begin{proof}
  ...
  inequality. Lemma~\ref{L4}, along with~\eqref{eq:17}
  ...
  Lemma~\ref{L2} then implies that
  the first term in \eqref{eq:38} converges to zero in probability.
  Lemma~\ref{L4} proves that the second and third
...
\begin{proof}[Proof of~(\ref{eq:27})]
  ...
  \end{multline}
  almost surely.
  By Lemma~\ref{L6} (Equation~\ref{eq:32}), for
  ...
  Lemma~\ref{L1}.
  Lemma~\ref{L7} ensures that there
  ...
  \end{equation*}
  for large enough $n$,%
\footnote{Lemma~\ref{L6}
  and~\ref{L7} are stated for the
...
\begin{proof}
  ...
  (\citealp{Dav:94}, 12.9.)
  Lemma~\ref{L7} ensures uniform integrability of
...
\footnote{The novelty in this part of the proof is the measurability
  ...
  For the second term on the \allcaps{RHS}~\eqref{eq:6}, use the sequence of inequalities
  \begin{align*}
...
Lemma~\ref{L6} applies here as well: the bounds

...
%DIF DEL Paper_2016-08-28.tex   Mon Jan  9 22:21:32 2017
%DIF ADD Paper.tex              Mon Jan  9 22:20:34 2017
...
(And define $\Pm^*(\cdot) = \pr^*[\cdot \mid \Ms]$, \DIFdelbegin \DIFdel{$\Em(\cdot)$}\DIFdelend
\DIFaddbegin
\DIFadd{$\Em^*(\cdot)$}
\DIFaddend$ = \E^*(\cdot \mid \Ms)$, etc.) An important property is
...
other random variables.\footnote{%DIF >
\DIFaddbegin
  \DIFadd{A referee suggested the following clarification:}
  \begin{itemize}
  \item[$\circ$] \DIFadd{$\Em^*(\cdot)$ denotes the expectation over the $u_{ni}$, holding
    the sample and $M_{ni}$ fixed.}
  \item[$\circ$] \DIFadd{$\Em(\cdot)$ denotes the expectation over the sample and
    $u_{ni}$, holding only the $M_{ni}$ fixed.}
  \item[$\circ$] \DIFadd{$\E^*(\cdot)$ denotes the expectation over the $u_{ni}$ and
    $M_{ni}$, holding the sample fixed.}
  \end{itemize}
  \DIFadd{The same distinctions hold for the probabilities as well.}
\DIFaddend} %DIF >
This property is especially important because it allows us
...
\end{equation}
almost surely for any function $g$ \DIFaddbegin \DIFadd{such that both
expressions are well-defined}\DIFaddend\ and any $j \leq k$. Equation~\eqref{eq:17}
...
%DIFAUXCMD
\DIFdel{The first equality follows from simple algebra and the definition
of the $u_{nj}$'s. The first inequality is a
consequence of~\eqref{eq:5D}.  The second equality holds by
monotonicity of $B$. And convergence in probability holds by
Lemma~\ref{L1}, completing the proof
of~\eqref{eq:19}.} \DIFdelend

\DIFaddbegin
\DIFadd{But note that
}\begin{equation*}
  \DIFadd{\sum_{j=1}^{J_n} \Em \big ( Z_{nj}^{*2} \ind \{ Z_{nj}^{*2} > \epsilon \} \big )
  \leq \max_{j=1,\dots,J_n} \Em \big ( \tfrac{ n }{ M_{nj} } Z_{nj}^{*2}
  \ind \{ \tfrac{ n }{ M_{nj} } Z_{nj}^{*2} >
  \min_{k=1,\dots,J_n} \tfrac{ n }{ M_{nk} } \epsilon \} \big )
}\end{equation*}
\DIFadd{almost surely. Lemmas~\ref{L1} and~\ref{L6} imply that the RHS converges to
zero in probability as $n \to \infty$,
proving~\eqref{eq:19}.} The Dominated Convergence Theorem and~(\ref{eq:18})
then ensure that
...
and~\eqref{eq:2} \DIFdelbegin \DIFdel{though an argument attributed to Poly{\`a}}
\DIFdelend \DIFaddbegin \DIFadd{since the limiting distribution is continuous,
a standard argument}\DIFaddend\ that proceeds as follows \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[see][Lemma 2.11, for example]{Vaa:00}}%DIFAUXCMD
}\DIFaddend .
...
\begin{proof}[Proof of Lemma~\ref{L1}]
  To prove part 1,
  \DIFdelbegin
  \DIFdel{observe that, for any increasing positive sequence
  $x_n$ such that $x_n p_n \to \infty$,}
  \begin{equation*}
    \DIFdel{\pr\Big[\max_{i=1,\dots, \lfloor C n p_n \rfloor} M_{ni} \leq x_n\Big] =
    (1 - (1 - p_n)^{x_n})^{\lfloor C n p_n \rfloor} \to \lim \exp(-C n p_n (1 - p_n)^{x_n})}
  \end{equation*}
  \DIFdel{and $C n p_n (1 - p_n)^{x_n} \to \lim C n p_n e^{-x_n p_n}$. Now, let
  $x_n = n x$ for} \DIFaddbegin \DIFadd{take} \DIFaddend any positive number $x$.  Then
  \begin{multline*}
    \pr\Big[\max_{i=1,\dots,\lfloor C n p_n \rfloor} M_{ni}/n \leq x \Big]
    \DIFadd{= (1 - (1 - p_n)^{n x})^{\lfloor C n p_n \rfloor}} \\
    \DIFdel{\to \lim} \DIFadd{=} \exp(-C n p_n e^{-n p_n x})
    \DIFadd{\times (1 + o(1))}
    \DIFdel{= \exp(0) = 1.}
  \end{multline*}
  \DIFdelend
  \DIFaddbegin
  \DIFadd{since $(1 - p_n)^{x/p_n} \to e^{-x}$. In addition, $n p_n e^{-n p_n} \to 0$
  by assumption, so}
  \begin{align*}
    \DIFadd{\pr\Big[\max_{i=1,\dots, \lfloor C n p_n \rfloor} M_{ni}/n \leq x \Big]
    \to \exp(0) = 1.}
  \end{align*}
  \DIFaddend
...
\begin{lem}\label{L6}
  \DIFaddbegin \DIFdelbegin
  Under the conditions of Theorem~\ref{T1}, \DIFaddbegin \DIFadd{for any positive
  $\epsilon$ and $\epsilon'$ there exist constants $C_0$ and $n_0$
  such that}
  \begin{multline}\label{eq:30}
    \DIFdel{\lim_{C \to \infty} \limsup_{n \to \infty}}
     \DIFadd{ \pr \Big [ } \sup_{\substack{\tau = 0,\dots,n-1 \\ m'=1,\dots,n}}
     \E_\DIFadd{\mathcal{M}} \Big ( \max_{ m = 1, \dots, m' } (n \, Z^*_n(\tau, m)^2 / m' ) \\
     \times \ind\Big\{\max_{m = 1,\dots,m'} n \, Z^*_n(\tau, m)^2 / m' > C\Big\}\Big)
    \DIFdel{= 0.}
  \DIFadd{> \epsilon \Big ] } \DIFadd{< \epsilon'}
  \end{multline}
  \DIFadd{for all $n > n_0$ and $C > C_0$.} Moreover, the families of random variables \\
  $\{\max_{m=1,\dots,m'} Z^{*}_n(\tau, m)^2 n / m'; \tau, m', n\}$ and
  \DIFdel{$\{\max_{m=1,\dots,M_{nj}} Z^{*}_{nj}(u_{nj}, m)^2 n / M_{nj}; j, n\}$ } \\
  \DIFadd{$\{Z^{*2}_{nj} n / M_{nj}; \tau, j, n\}$ } are
  uniformly integrable \DIFadd{for $n$ larger than some threshold $n_0$}.
   \DIFaddend
\end{lem}
...
\begin{proof}
  ...
  integrability of \DIFdel{the first family of random variables is simply a restatement of
  that expression and
  uniform integrability of} $Z_{nj}^{*2} n / M_{nj}$
  \DIFdel{follows an abbreviated version of the proof of~\eqref{eq:30}.}
  \DIFadd{and
...
\DIFdel{First we will show that, for every $\epsilon > 0$ and large enough
  $n$, there exists an $\epsilon' > 0$ with the property that
  }\begin{equation}\label{eq:31D} \tag{\DIFdel{40}} \DIFdel{
    \E \Big(\ind(A) \times \max_{m = 1,\dots,m'} Z^*_n(\tau, m)^2 n / m'\Big) < \epsilon
  }\end{equation}
  %DIFAUXCMD
\DIFdel{for any event $A \in \Fs_n \equiv \sigma(X_{n1},\dots,X_{nn})$ with $\pr(A) \leq \epsilon'$. Then we will show that this property
  implies uniform integrability.}\footnote{%DIF <
    \DIFdel{The novelty in this part of the proof is the measurability
    ...
    integrability would be an immediate consequence of~\eqref{eq:31D}
    and would not need to be shown separately. See \mbox{%DIFAUXCMD
\citet[Theorem
    12.9]{Dav:94}}%DIFAUXCMD
.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD

%DIFDELCMD <   %%%
\DIFdel{Take an arbitrary $\delta > 0$ and a value of $\delta' > 0$
  so that}
  \begin{equation}\label{eq:35D} \tag{\DIFdel{41}}
    \DIFdel{\sup_{m'=1,\dots,n} \E \Big(\ind(A) \max_{m = 1,\dots,m'}
    Z_n'(\tau, m)^2 n / m' \Big) < \delta
  }\end{equation}
  %DIFAUXCMD
\DIFdel{for any $A \in \Fs_n$ with $\pr(A) \leq \delta'$; the existence
  of $\delta'$ is ensured by Lemma~\ref{L8}.}
For any $x$, define $J(x)$ to be the block index such that $K_{n,J(x) - 1} < x
  ...
  \begin{multline} \label{eq:42}
    ...
    + Z_n^*(K_{n,J(\tau+m)-1}, m - K_{n,J(\tau + m)-1}) \DIFadd{.}
  \end{multline}

  \DIFdel{giving}
  \begin{align}
  \DIFdel{\E  \Big(}&\DIFdel{\ind(A) \times \max_{m = 1,\dots,m'} n \, Z^*_n(\tau, m)^2 / m'\Big)
  \notag }\\
  &\DIFdel{\leq 3 \E\Big[ \Em^{*} \Big[\ind(A) \tfrac{n}{m'}\max_{m = 1,\dots, m'}
  \Big(Z_n^*(\tau, K_{n,J(\tau)} - \tau) + \sum_{j=J(\tau)+1}^{J(\tau + m)-1} Z_{nj}^*\Big)^2\Big]
  \notag }\\
  &\DIFdel{\quad + 3 \Em^{*} \big[\ind(A) \tfrac{n}{m'}
  \max_{m=1,\dots,m'} Z_n^*(K_{n,J(\tau+m)-1}, m - K_{n,J(\tau + m)-1})^2 \big]\Big]}
    \label{eq:6D} \tag{\DIFdel{42}}
  \end{align}
  %DIFAUXCMD
  \DIFdel{almost surely.}%DIFDELCMD <
  \DIFadd{Since the distribution of $Z_n^*(\tau, m)$ does not depend on
  $\tau$, \eqref{eq:30} holds if, for any positive $\epsilon$ and
  $\epsilon'$, there exist $n_0$ and $C_0$ such that
  }\begin{multline} \label{eq:31} \DIFadd{
    \Pr \big [ \max_{\tau=0,\dots,n-1}
    \Em \big ( ( \max_{ m = 1, \dots, K_{n,J(\tau)} - \tau} n \, Z^*_n(\tau, m)^2 / M_{n,J(\tau)} ) }\\
    \DIFadd{\times \ind \{ \max_{ m = 1, \dots, K_{n,J(\tau)} - \tau} n \, Z^*_n(\tau, m)^2 / M_{n,J(\tau)} > C \}
    \big ) > \epsilon \big ] < \epsilon',
  }\end{multline}
  \begin{multline} \DIFadd{\label{eq:44}
    \pr \Big [
    \max_{m'=1,\dots,n}
    \Em \Big (
  ...
  \DIFadd{and
  }\begin{multline} \label{eq:5} \DIFadd{
    \Pr \big [ \max_{j=1,\dots,J_n}
    \Em \big ( ( \max_{ m = 1, \dots, M_{nj}} n \, Z^*_n(K_{n,j-1}, m)^2 / M_{nj} ) }\\
    \DIFadd{\times \ind \{ \max_{ m = 1, \dots, M_{nj}} n \, Z^*_n(K_{n,j-1}, m)^2 / M_{nj} > C \}
    \big ) > \epsilon \big ] < \epsilon',
  }\end{multline}
  \DIFadd{for all $n > n_0$ and $C > C_0$.}

%DIFDELCMD <   %%%
\DIFdel{Since $Z_{nj}^*$ is a martingale difference array, Doob's maximal
  inequality for martingales \mbox{%DIFAUXCMD
\citep[see][Theorem 15.15, for
  example]{Dav:94} }%DIFAUXCMD
along with the construction of $Z_{nj}^{*}$ gives
  ...
  \end{align}
  %DIFAUXCMD
  \DIFdel{the third follows by construction of $A$
  and the fourth follows from an argument similar to Lemma~\ref{L3}. }%DIFDELCMD <
  \DIFadd{to get the upper bound
  }\begin{align*}
    \DIFadd{\Em \big ( }& \DIFadd{( \max_{ m = 1, \dots, M_{nj} } n \, Z^*_n(K_{n,j-1},m)^2 / M_{nj} )
    \ind \{ \max_{ m = 1, \dots, M_{nj} } n \, Z^*_n(K_{n,j-1}, m) ^2 / M_{nj} > C \} \big ) }\\
    & \DIFadd{\leq 3 \Em \big ( ( \max_{ m = 1, \dots, M_{nj} } n \, Z'_n(u_n,m)^2 / M_{nj} )
    \ind \{ \max_{ m = 1, \dots, M_{nj} } n \, Z'_n(u_n,m)^2 / M_{nj} > C / 3 \} \big ) }\\
    & \DIFadd{\quad + 3 \Em \big ( ( M_{nj} Z'_n(0,n)^2 / n )
    \ind \{ M_{nj} Z'_n(0,n)^2 / n > C / 3 \} \big )
    + 3 \tfrac{M_{nj}}{n} \sum_{t=1}^n ( \mu_{nt} - \bar \mu_n ) ^2,
  }\end{align*}
  \DIFadd{where $u_n$ has the discrete uniform distribution over $0,\dots,n-1$.
  Lemmas~\ref{L1}, \ref{L3}, and \ref{L8} ensure that there exist
  $n_0$ and $C_0$ such that, for all $n > n_0$ and $C > C_0$,
  }\begin{equation*}
    \DIFadd{\Em \big ( (\max_{ m = 1, \dots, M_{nj} } n \, Z'_n(u_n,m)^2 / M_{nj} )
    \ind \{\max_{ m = 1, \dots, M_{nj} } n \, Z'_n(u_n,m)^2 / M_{nj} > C / 3 \}
    \big ) < \epsilon / 9
  }\end{equation*}
  \DIFadd{and
  }\begin{equation*}
    \DIFadd{\Em \big ( ( M_{nj} \, Z'_n(0,n)^2 / n )
    \ind \{ M_{nj} \, Z'_n(0,n)^2 / n > C / 3 \} \big )
    < \epsilon / 9
  }\end{equation*}
  \DIFadd{for all $j = 1,\dots,J_n - 1$ almost surely, and
  }\begin{equation*}
    \DIFadd{\pr \Big [ \max_{j=1,\dots,J_n-1}
    \tfrac{M_{nj}}{n} \sum_{t=1}^n ( \mu_{nt} - \bar \mu_n )^2 > \epsilon / 9 \Big ]
    < \epsilon' / 3,
  }\end{equation*}
  \DIFadd{proving~\eqref{eq:5}. The proof of~\eqref{eq:31} is essentially the same.}


%DIFDELCMD <   %%%
\DIFdel{For the second term on the \allcaps{RHS} of~\eqref{eq:6D},
...
\DIFdel{for constants $C_1'$ and $C_2'$. To express these bounds in the form
  ...
  \DIFadd{The proof of~\eqref{eq:44} follows a streamlined version of \mbox{%DIFAUXCMD
\citet[Lemma
  6.5]{Mcl:75b} }%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet[Lemma 3.5]{Mcl:77}}%DIFAUXCMD
. (Those results are
  presented as Theorem~16.13 in \mbox{%DIFAUXCMD
\citealp{Dav:94}}%DIFAUXCMD
, and are largely
  reproduced in the proof of this paper's Lemma~\ref{L8}.)}

  \DIFadd{Separate $Z_{nj}^*$ into two components,}
  \begin{align*}
    \DIFadd{U_{nj}^* }&\DIFadd{= (\tfrac{n}{m'})^{1/2} \, Z_{nj}^*
    \ind\{\tfrac{n}{M_{nj}} \, Z_{nj}^{*2} > C' \} -
    (\tfrac{n}{m'})^{1/2} \Em^*( Z_{nj}^*
    \ind\{\tfrac{n}{M_{nj}} Z_{nj}^{*2} > C' \}) }\\
    \DIFadd{Y_{nj}^* }&\DIFadd{= (\tfrac{n}{m'})^{1/2} \, Z_{nj}^*
    \ind\{\tfrac{n}{M_{nj}} \, Z_{nj}^{*2} \leq C' \} -
    (\tfrac{n}{m'})^{1/2} \Em^*( Z_{nj}^*
    \ind\{\tfrac{n}{M_{nj}} Z_{nj}^{*2} \leq C'\}),}
    \intertext{\DIFadd{making}}
    \DIFadd{Z_{nj}^* }&\DIFadd{= (\tfrac{m'}{n})^{1/2} U_{nj}^* + (\tfrac{m'}{n})^{1/2} Y_{nj}^*}
  \end{align*}
  \DIFadd{where $C'$ is a constant that will be constrained later in the
  proof. For any $C$, $\tau$, $m'$, $n$, standard probabilistic
  inequalities~(e.g. Theorem 2.29 of \mbox{%DIFAUXCMD
\citealp{Dav:94}}, along with
  maximal inequalities for martingale difference sequences) imply that
  }
  \begin{multline}
    \DIFadd{\Em \Big ( \tfrac{n}{m'} \max_{k=1,\dots,J(m')}
    ...
    \DIFadd{\tfrac{6}{C} \tfrac{4^4}{3^4}
    \Em \Big \lvert \sum_{j=1}^{J(m')} Y_{nj}^* \Big \rvert^4
    + 24 \Em \Big \lvert \sum_{j=1}^{J(m')} U_{nj}^* \Big \rvert^2}
    \label{eq:43}
  \end{multline}
  \DIFadd{almost surely.}

  \DIFadd{The first term on the RHS of~\eqref{eq:43} can be
  expanded recursively to give the following upper bound
  }\begin{align*}
    \DIFadd{\tfrac{1}{C} \Em \Big\lvert \sum_{j=1}^{J(m')} Y_{nj}^* \Big\rvert^4
    }& \DIFadd{\leq \Em \Big[ \tfrac{1}{C} \Em^* \Big\lvert \sum_{j=1}^{J(m')-1} Y_{nj}^* \Big\rvert^4
    + \tfrac{6}{C} \Em^* \Big( Y_{n,J(m')}^{*2} \Big\lvert
    \sum_{j=1}^{J(m')-1} Y_{nj}^* \Big\rvert^2 \Big) }\\
    & \DIFadd{\quad + \tfrac{4}{C} \Em^* \Big\lvert Y_{n,J(m')}^{*3} \sum_{j=1}^{J(m')-1} Y_{nj}^* \Big\rvert
    + \tfrac{1}{C} \Em^* \lvert Y_{n,J(m')}^{*4} \rvert \Big] }\\
    & \DIFadd{\leq \tfrac{1}{C} \Em^* \Big ( \sum_{j=1}^{J(m')-1} Y_{nj}^* \Big ) ^4
    + \tfrac{1}{C} (\tfrac{4 C'}{m'})^2
    \Big( 6 M_{n,J(m')} \sum_{j=1}^{J(m')-1} M_{nj} }\\
    & \DIFadd{\quad + 4 M_{n,J(m')}^{3/2} \Big [ \sum_{j=1}^{J(m')-1} M_{n,J(m')} \Big ] ^{1/2}
    + M_{n,J(m')}^2 \Big ) }\\
    & \DIFadd{\leq \tfrac{ 1 }{ C } ( \tfrac{ 4 C' }{ m' } )^2 \sum_{j=2}^{J(m')}
      \Big ( 6 M_{nj} \sum_{ k = 1 }^{ j - 1 } M_{nk}
      + 4 M_{nj}^{3/2} \Big [ \sum_{ k = 1 }^{ j - 1 } M_{ nk } \Big ] ^{1/2}
      + M_{nj}^2 \Big ) }\\
    & \DIFadd{\leq \tfrac{11}{C} \tfrac{ K _{ n, J(m') } }{ m' } (4 C')^2
  }\end{align*}
  \DIFadd{with all inequalities holding almost surely.
  ...
  \DIFadd{The second term on the RHS of~\eqref{eq:43} satisfies
  }\begin{align*}
    \DIFadd{\Em \Big \lvert \sum_{j=1}^{J(m')} U_{nj}^* \Big \rvert^2
    }& \DIFadd{= \sum_{j=1}^{J(m')} \Em \lvert U_{nj}^* \rvert^2 }\\
    & \DIFadd{\leq \max_{j=1,\dots,J(m')} \Em \big(
      \tfrac{n}{M_{nj}} Z_{nj}^{*2}
      \ind\{ \tfrac{n}{M_{nj}} Z_{nj}^{*2} > C' \} \big) .
  }\end{align*}

  \DIFadd{Equation~\eqref{eq:5} ensures that we can choose $C'$ and $n'$ such that}
  \begin{equation*}
    \DIFadd{\pr \Big [
    ...
    < \epsilon' / 2}
  \end{equation*}
  \DIFadd{for all $n > n'$ and $C > C'$. We can then choose $n_0 > n'$ and $C_0 > C'$
  such that}
  \begin{equation*}
    \DIFadd { \pr \Big [
    \max_{ m' = 1, \dots, n }
  ...
  \end{equation*}
  \DIFadd{for all $n > n_0$ and $C > C_0$, completing the proof.}
  \DIFaddend
  \DIFdelend
 \end{proof}

...
%DIF ADD Paper.tex              Wed Sep 14 16:19:05 2016
...
\DIFadd{we have defined $D_n$ to minimize notation in the subsequent
...
\DIFaddbegin \DIFadd{Observe that
}\[
\DIFadd{\sqrt{n}(\bar X_{n}^{*} -  \E^{*} \bar X_n^{*})
= \sqrt{n}((\bar{X}_{n}^{*} - \bar{\mu}_{n}^{*}) - \E^{*} (\bar X_n^{*} - \bar{\mu}_n^{*}))
+ \sqrt{n}(\bar{\mu}_{n}^{*} - \E^{*} \bar{\mu}_n^{*}).
}\]
\DIFadd{Lemma~\ref{L3} implies that the last term is $o_{L_2}(1)$, so we can assume that
...
Lemma~\ref{L6},
Equation~\eqref{eq:30}. Since $J_n$ and $M_{nj}$ are both $\Ms$\DIFdelbegin \DIFdel{-measureable
...
\citep[Lemma 6]{CRT:65}
  ...
  \DIFadd{Then $\E J_n = (n-1) p_n + 1$, completing the proof.
}\DIFaddend \end{proof}
...
\begin{proof}
  \DIFadd{Since the blocks are independent and
  ...
  show~\eqref{eq:40}, observe that
    ...
    &\DIFadd{= \E\Big(\sum_{j=1}^{J_n}\sum_{s,t = K_{n,j-1}+1}^{K_{nj}}
  ...
  \DIFadd{The first line follows from the LIE. The second exploits the
...
\DIFdelend \DIFaddbegin \eqref{eq:25}\DIFaddend , $(Z_{nj}^{*2} - \Em^* Z_{nj}^{*2}) \cdot (n /
  ...
  %DIFAUXCMD
\DIFdel{after several applications of }\DIFdelend \DIFaddbegin \begin{multline*}
  ...
  holds in $L_1$ \DIFdelbegin \DIFdel{as well }\DIFdelend and Lemma~\ref{L2} then implies that
...
\DIFdelend \DIFaddbegin \E \DIFaddend \Big(\ind(A) \times \max_{m = 1,\dots,m'} Z^*_n(\tau, m)^2 n / m'\Big) < \epsilon
  ...
  implies uniform integrability.\DIFaddbegin \footnote{%DIF > 
    \DIFadd{The novelty in this part of the proof is the measurability
    ...
    to be shown separately. See \mbox{%DIFAUXCMD
\citet[Theorem 12.9]{Dav:94}}%DIFAUXCMD
.}} \DIFaddend %
\DIFdelbegin \footnote{\DIFdel{The novelty in this part of the proof is the measurability
  ...
  proved~}%DIFDELCMD < \eqref{eq:31} %%%
\DIFdel{for all $A \in \Omega_n$, as we did for the
...
\DIFdelend 
  ...
  Take an arbitrary \DIFdelbegin \DIFdel{$\epsilon > 0$ }\DIFdelend \DIFaddbegin \DIFadd{$\delta > 0$ }\DIFaddend and a value of \DIFdelbegin \DIFdel{$\epsilon' > 0$
  so that
  }\begin{displaymath}
  ...
  %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{$\delta' > 0$
  so that
  }\begin{equation}\DIFadd{\label{eq:35}
  ...
  giving
  \begin{align}
  \DIFdelbegin %DIFDELCMD < \Em \Em%%%
\DIFdel{^* }\DIFdelend \DIFaddbegin \E  \DIFaddend \Big(&\ind(A) \times \max_{m = 1,\dots,m'} n \, Z^*_n(\tau, m)^2 / m'\Big)
  \notag \\
  &\leq 3 \DIFaddbegin \E\Big[ \DIFaddend \Em\DIFdelbegin %DIFDELCMD < \Em%%%
\DIFdel{^* }\DIFdelend \DIFaddbegin \DIFadd{^{*} }\DIFaddend \Big[\ind(A) \tfrac{n}{m'}\max_{m = 1,\dots, m'}
  \Big(Z_n^*(\tau, K_{n,J(\tau)} - \tau) + \sum_{j=J(\tau)+1}^{J(\tau + m)-1} Z_{nj}^*\Big)^2\Big]
  \notag \\
  &\quad + 3 \Em\DIFdelbegin %DIFDELCMD < \Em%%%
\DIFdel{^* }\DIFdelend \DIFaddbegin \DIFadd{^{*} }\DIFaddend \big[\ind(A) \tfrac{n}{m'}
  ...
  \DIFdelbegin \begin{multline*}
  \DIFdel{\Em \Em^* \Big[\ind(A) \tfrac{n}{m'}\max_{m = 1,\dots, m'}
  \Big(Z_n^*(\tau, K_{n,J(\tau)} - \tau) + \sum_{j=J(\tau)+1}^{J(\tau + m)-1} Z_{nj}^*\Big)^2\Big] }\\
  \DIFdel{\leq \tfrac{1}{m'} \sum_{u = 0}^{n-1} \Big\{\Em\big[\ind(A) Z_n(u, K_{n,J(\tau)} - \tau)^2\big] +
  \sum_{j=J(\tau)+1}^{J(\tau + m')-1} \Em \big[\ind(A) Z_n(u, M_{nj})^2}]\DIFdel{\Big\}
  }\end{multline*}
  %DIFAUXCMD
\DIFdel{and this term is less than $\epsilon/6$ }\DIFdelend \DIFaddbegin \begin{align*}
  \DIFadd{\E \Em^* }& \DIFadd{\Big[\ind(A) \tfrac{n}{m'}\max_{m = 1,\dots, m'}
  \Big(Z_n^*(\tau, K_{n,J(\tau)} - \tau)
  + \sum_{j=J(\tau)+1}^{J(\tau + m)-1} Z_{nj}^*\Big)^2\Big] }\\
  &\DIFadd{\leq \tfrac{1}{m'} \sum_{u = 0}^{n-1} \E \Big[\Em \big[
  ...
  &\DIFadd{\quad+ 3(\delta'/n) \E\Big( \Big(\sum_{t \in I_n(u, K_{n,J(\tau)} - \tau)}
  (\mu_{nt} - \bar \mu_n)\Big)^2 +
  \sum_{j=J(\tau)+1}^{J(\tau + m')-1}\Big( \sum_{t \in I_n(u,M_{nj})}
  ...
  \DIFdelbegin \DIFdel{.
  }\DIFdelend \DIFaddbegin \DIFadd{and fourth follows from an argument similar to Lemma~\ref{L3}.
}
...
\DIFdelend \DIFaddbegin \tfrac{1}{m'} \DIFaddend \sum_{u=0}^{n-1} \Em \big[\ind(A)
  \max_{m=1,\dots,\min(M_{nj}, m' - K_{n,j-1})} Z_n(u, m)^2 \DIFdelbegin \DIFdel{/ M_{nj} }\DIFdelend \big]\DIFaddbegin \Big) \DIFaddend \\
  &\leq \DIFaddbegin \E \Big(\DIFaddend \sum_{j=J(\tau) + 1}^{J(\tau+m')} \DIFdelbegin \DIFdel{\epsilon \cdot \min}\DIFdelend \DIFaddbegin \tfrac{3}{m'} \DIFadd{\sum_{u=0}^{n-1} }\Big\{
  \Em \big[\ind\DIFaddend (\DIFdelbegin \DIFdel{M_{nj}}\DIFdelend \DIFaddbegin \DIFadd{A)
  \max_{m=1,\dots,\min(M_{nj}, m' - K_{n,j-1})} Z_n'(u}\DIFaddend , m\DIFdelbegin \DIFdel{' }\DIFdelend \DIFaddbegin \DIFadd{)^2 }\big] \\
  &\quad \DIFadd{+
  }\tfrac{1}{n} \Pm[A] \DIFadd{\max_{m = 1,\dots,\min(M_{nj}, m'-K_{n,j-1})}
  }\Big(\DIFadd{\sum_{t \in I_n(u,m)} (\mu_{nt} }\DIFaddend - \DIFdelbegin \DIFdel{K_{n,j-1}}\DIFdelend \DIFaddbegin \bar{\mu}\DIFadd{_n}\DIFaddend )\DIFdelbegin \DIFdel{/ 6 m}\DIFdelend \DIFaddbegin \Big)\DIFadd{^2 }\\
  &\quad\DIFadd{+ }\tfrac{\min(M_{nj},m'-K_{n,j-1})^2}{n^2} \Em\DIFadd{( }\ind\DIFadd{(A) Z_n}\DIFaddend '\DIFaddbegin \DIFadd{(u, n)^2 ) }\Big\}\Big)\DIFaddend \\
  ...
  \begin{equation*}
    \DIFdelbegin %DIFDELCMD < \Pm%%%
\DIFdelend \DIFaddbegin \DIFadd{\Pr}\DIFaddend \Big[ \max_{m = 1,\dots,m'} n \, Z^*_n(\tau, m)^2 / m' > C_0 \Big] \leq \epsilon'.
  \end{equation*}
  Markov's inequality guarantees the existence of this $C_0$. Then
  \begin{equation*}
    \DIFdelbegin %DIFDELCMD < \Em %%%
\DIFdelend \DIFaddbegin \E \DIFaddend \Big(\max_{m = 1,\dots,m'} n \, Z^*_n(\tau, m)^2 / m'

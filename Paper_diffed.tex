...
%DIF DEL Paper_2015-12-31.tex   Wed Sep 14 16:18:38 2016
%DIF ADD Paper.tex              Mon Sep 26 09:37:57 2016
...
requirement that it be $o(n)$\DIFaddbegin \DIFadd{, although, as we will show, their
stronger assumption is necessary for the bootstrap to be consistent
under some forms of heterogeneity}\DIFaddend .
...
\begin{thm}\label{T1}
  ...
  \item The array $\mu_{nt} - \bar \mu_n$ is uniformly bounded
    and \DIFdelbegin \DIFdel{$\sum_{t=1}^n (\mu_{nt} - \bar \mu_n)^2 \to 0$}\DIFdelend \DIFaddbegin \DIFadd{$\sum_{t=1}^n (\mu_{nt} - \bar \mu_n)^2 = o(n p_n^2)$ (for
    the Stationary Bootstrap) or
    $\sum_{t=1}^n (\mu_{nt} - \bar \mu_n)^2 = o(n/M_n^2)$ (for
    the Moving or Circular Block Bootstraps)}\DIFaddend ,
    where $\E X_{nt} = \mu_{nt}$\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend $\bar{\mu}_n = n^{-1} \sum_{t=1}^n
    \mu_{nt}$\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, and $p_n$ and $M_n$ are further defined in item 4.
    }\DIFaddend Moreover, $\sqrt{n} \| \bar{X}_{n} - \bar\mu_n \|_2
  ...
  \end{enumerate}
  Then \DIFaddbegin \DIFadd{$\sigma^{*2} \to^p \sigma^2$, }\DIFaddend $\hat\sigma^{*2} \to^p \sigma^2$,
...
\DIFdelbegin \DIFdel{Similarly, our }\DIFdelend \DIFaddbegin 

\DIFadd{The }\DIFaddend assumption on the dispersion of the individual means,
$(\mu_{nt} - \bar\mu_n)^2$, is \DIFdelbegin \DIFdel{slightly stronger than }\DIFdelend \DIFaddbegin \DIFadd{the same as }\DIFaddend \citepos{GoW:02} and
\citepos{GoJ:03} \DIFdelbegin \DIFdel{to
accommodate larger block sizes.%DIF < 
}\footnote{\DIFdel{Note that this is one reason that we are presenting our
  results using general triangular-array notation: to allow for the
  means to change over time for comparison with \mbox{%DIFAUXCMD
\citepos{GoW:02} }%DIFAUXCMD
and
  \mbox{%DIFAUXCMD
\citepos{GoJ:03} }%DIFAUXCMD
results.}}
%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{under their additional requirement that the
expected block length be $o(\sqrt{n})$. When
the blocks are larger than $o(\sqrt{n})$, a case that \mbox{%DIFAUXCMD
\citet{GoW:02}
}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{GoJ:03} }%DIFAUXCMD
do not consider, the dispersion must be smaller
than they require.
...
\DIFadd{A small amount of algebra gives the relationship,
}\begin{equation*}
  \DIFadd{\sum_{t=1}^n (\mu_{nt} - \bar \mu_n)^2
  = T_n (n - T_n) \Delta_n^2 / n \equiv D_n;
}\end{equation*}
\DIFadd{we have introduced $D_n$ to minimize notation in the subsequent
...
estimated \mbox{%DIFAUXCMD
\citep[as in][]{Bai:94}}%DIFAUXCMD
, but the break is ignored and the
bootstrap is applied to the entire dataset.
}\DIFaddend 

\DIFaddbegin \DIFadd{To see when Theorem~\ref{T1}'s Assumption~2 is satisfied, consider two
...
\DIFaddend Theorem~\ref{T1} relies on a general insight about the
...
(X^*_{nt} - \DIFdelbegin \DIFdel{\mu_n^{*}}\DIFdelend
\DIFaddbegin \DIFadd{\E^* \bar X_n^*}\DIFaddend).
\end{align*}
...
\DIFdelbegin \DIFdel{$\sup_{t=1,\dots,n}|\mu_{nt} -
\bar \mu_n| = o(1/\sqrt{n})$ and
}\DIFdelend \begin{equation}\label{eq:4}
...
\begin{align}\DIFdelbegin %DIFDELCMD < \label{eq:4}
%DIFDELCMD <    %%%
\DIFdelend \DIFaddbegin \label{eq:39}
...
By construction, (see Equations~\DIFdelbegin %DIFDELCMD < \eqref{eq:4} %%%
\DIFdelend \DIFaddbegin \eqref{eq:39} \DIFaddend and~\eqref{eq:1})
...
Lemma~\ref{L6}\DIFaddbegin\DIFadd{.}\DIFaddend%
\DIFdelbegin\DIFdel{, Equation~\eqref{eq:30}} \DIFdelend
Since $J_n$ and $M_{nj}$ are both $\Ms$\DIFdelbegin \DIFdel{-measureable
}\DIFdelend \DIFaddbegin \DIFadd{-measurable
}\DIFaddend and independent of the $u_{nj}$'s and $X_{nt}$'s,~\eqref{eq:5} implies that
\begin{align*}
...
\end{align*}
The first equality follows from simple \DIFdelbegin \DIFdel{algerbra }\DIFdelend \DIFaddbegin \DIFadd{algebra }\DIFaddend and the definition
...
\begin{lem}\label{L1}
  ...
  \ell_n^{1+\epsilon} \to^p 0$ as $n \to \infty$ for any positive
  $\epsilon$, and
  \item \DIFdelbegin \DIFdel{$\sum_{i=1}^{J_n} M_{ni}^2 / n^2 \to^p 0$}\DIFdelend \DIFaddbegin \DIFadd{$\E\big(\sum_{i=1}^{J_n} M_{ni}^2\big) = (2 n - 1) / p_n - n + 1$}\DIFaddend .
  \end{enumerate}
\end{lem}
...
%DIFDELCMD <   %%%
\DIFdelend The proof of part 4 is the same as part 2, making the obvious
  substitutions.
\DIFdelbegin \DIFdel{Part }\DIFdelend \DIFaddbegin 

  \DIFadd{For part }\DIFaddend 5\DIFdelbegin \DIFdel{holds because $\sum_{i=1}^{J_n} M_{ni}^2/n^2
  \leq \max_{i=1,\dots,J_n} M_{ni}/n$ which converges to zero in
  probability by part 2.
}\DIFdelend \DIFaddbegin \DIFadd{, write
  }\begin{equation}\DIFadd{\label{eq:41}
  ...
  \DIFadd{so
    ...
    \DIFadd{\E \Big( \sum_{i=1}^{J_n} M_{ni}^2 \Big)
    = \E\Big( \sum_{i=1}^{J_n} (M_{ni} - \tfrac{1}{p_n})^2 \Big)
    + \tfrac{2 n}{p_n} - \E \tfrac{J_n}{p_n^2}.
  }\end{equation*}
  \DIFadd{Since $J_n$ is a stopping time and $\sum_{i=1}^j (M_{ni} - 1/p_n)$
  is a martingale, we have the equality \mbox{%DIFAUXCMD
\citep[Lemma 6]{CRT:65}
  }%DIFAUXCMD
}\begin{align*}
    \DIFadd{\E\Big( \sum_{i=1}^{J_n} (M_{ni} - \tfrac{1}{p_n})^2 \Big)
    }&\DIFadd{= \E\Big( \sum_{i=1}^{J_n} \E((M_{ni} - \tfrac{1}{p_n})^2 \mid M_{n1},\dots,M_{n,i-1})\Big)}\\
    &\DIFadd{= \tfrac{1 - p_n}{p_n^2} \E J_n.
  }\end{align*}
  \DIFadd{Finally, $\E J_n = (n-1) p_n + 1$, completing the proof.
}\DIFaddend \end{proof}
...
\begin{lem}\DIFaddbegin \label{L3}
\DIFadd{Under the conditions of Theorem~\ref{T1},
}\begin{equation}\DIFadd{\label{eq:40}
  \E \Big(\sum_{j=1}^{J_n} \Big( \sum_{t = K_{n,j-1}+1}^{K_{nj}}
  (\mu_{nt}^{*} - \bar \mu_n)\Big)^2\Big) = o(n p_n)
}\end{equation}
\DIFadd{and $\bar \mu_n^{*} = \E^{*} \bar \mu_n^{*} + o_{L_2}(n^{-1/2})$.
}\end{lem}
\begin{proof}
  \DIFadd{The second result, on $\bar\mu_n^*$, is an immediate implication of
  Equation~}\eqref{eq:40}\DIFadd{. To show~}\eqref{eq:40}\DIFadd{, observe that
  }\begin{align*}
    \DIFadd{\E \Big(\sum_{j=1}^{J_n} }&\DIFadd{\Big( \sum_{t= K_{n,j-1} +1}^{K_{nj}}
    (\mu_{nt}^{*} - \bar \mu_n)\Big)^2\Big)= }\\
    &\DIFadd{= \E\Big(\sum_{j=1}^{J_n} \big\lvert \sum_{s,t = K_{n,j-1}+1}^{K_{nj}}
    \E \big((\mu_{ns}^{*} - \bar{\mu}_n)
    (\mu_{nt}^{*} - \bar{\mu}_n) \mid \Ms \big) \big\rvert \Big)}\\
    &\DIFadd{\leq \E\Big( \sum_{j=1}^{J_n}
  ...
  summation
    ...
    \DIFadd{\sum_{\tau=0}^{n-1} (\mu_{n,\tau+k} - \bar{\mu}_n)^2
    1\{ \tau + k \in I_n(\tau, M_{nj})\}
  }\]
  \DIFadd{is zero for all but $M_{nj}$ values of $k$ and is a nonzero constant
  for those $k$. The last line holds by Lemma~\ref{L1} and assumption.
}\end{proof}

\begin{lem}\DIFaddend \label{L4}
...
\begin{proof}
  For~\DIFdelbegin %DIFDELCMD < \eqref{eq:26}%%%
\DIFdelend \DIFaddbegin \eqref{eq:25}\DIFaddend , $(Z_{nj}^{*2} - \Em^* Z_{nj}^{*2}) \cdot (n /
  ...
  \DIFdelbegin \begin{displaymath}
    \DIFdel{\Big| \sum_{j=1}^{J_n} (Z_{nj}^{*2} - Z_{nj}^{\prime*2}) \Big| \leq
    2 \Big(\sum_{j=1}^{J_n} Z_{nj}^{*2} \Big)^{1/2}
    \Big(\tfrac{1}{n} \sum_{t=1}^n (\mu_{nt} - \bar X_n)^2\Big)^{1/2}
    + \tfrac{1}{n} \sum_{t=1}^n (\mu_{nt} - \bar X_n)^2
  }\end{displaymath}
  %DIFAUXCMD
\DIFdel{after several applications of }\DIFdelend \DIFaddbegin \begin{align*}
    \DIFadd{\Big| \sum_{j=1}^{J_n} (Z_{nj}^{*2} - Z_{nj}^{\prime*2}) \Big|
    }&\DIFadd{= \Big| \sum_{j=1}^{J_n} \big(Z_{nj}^{*} - \big[Z_{nj}^{*} + \tfrac{1}{\sqrt{n}} \sum_{t=K_{n,j-1}+1}^{K_{nj}} (\bar X_n - \mu_{nt}^*)\big]^2\big) \Big| }\\
    &\DIFadd{\leq 2 \Big(\sum_{j=1}^{J_n} Z_{nj}^{*2} \Big)^{1/2}
    \Big(\tfrac{1}{n} \sum_{j=1}^{J_n} \Big( \sum_{t= K_{n,j-1} + 1}^{K_{nj}}
    (\mu_{nt}^{*} - \bar X_n)\Big)^2 \Big)^{1/2}}\\
    &\DIFadd{\quad + \tfrac{1}{n} \sum_{j=1}^{J_n} \Big( \sum_{t= K_{n,j-1} + 1}^{K_{nj}}
    (\mu_{nt}^{*} - \bar X_n)\Big)^2
  }\end{align*}
  \DIFadd{from }\DIFaddend the Cauchy-Schwarz inequality. Lemma~\ref{L5}, along
  with~\eqref{eq:17} and~\eqref{eq:25}, implies that
  $\sum_j Z_{nj}^{*2} = O_p(1)$\DIFdelbegin \DIFdel{;
  $(1/n) \sum_t (\mu_{nt} - \bar X)^2$ converges to zero in
  probability by assumption on $\mu_{nt} - \bar\mu_n$ and because
  }\DIFdelend \DIFaddbegin \DIFadd{. Lemma~\ref{L3} implies that
  }\[
    \DIFadd{\tfrac{1}{n} \sum_{j=1}^{J_n} \Big( \sum_{t= K_{n,j-1} + 1}^{K_{nj}}
    (\mu_{nt}^{*} - \bar X_n)\Big)^2 \to^p 0
  }\]
  \DIFadd{since }\DIFaddend $\bar X$ itself obeys the \lln.
...
\begin{lem}\label{L6}
  Under the conditions of Theorem~\ref{T1},
  \DIFdelbegin \begin{eqnarray*}\DIFdel{\label{eq:35}
    \lim_{C \to \infty} \limsup_{n \to \infty} \sup_{m' = 1,\dots,n}
    \E\Big(\Big(\sum_{\tau=0}^{n-1} \max_{m=1,\dots,m'} Z_n(\tau, m)^2 / m'\Big) }\\
    \DIFdel{\times\ind\Big\{\sum_{\tau=0}^{n-1} \max_{m=1,\dots,m'} Z_n(\tau, m)^2 / m' > C\Big\}\Big) = 0.
  }\end{eqnarray*}
  %DIFAUXCMD
\DIFdel{and
  }\DIFdelend \begin{multline}\label{eq:30}
    ...
    = 0.
  \end{multline}
\DIFdel{making the family }\DIFdelend \DIFaddbegin \DIFadd{Moreover, the families }\DIFaddend of random variables
  $\{\max_{m=1,\dots,m'} Z^{*}_n(\tau, m)^2 n / m'; \tau, m', n\}$ \DIFaddbegin \DIFadd{and
  $\{\max_{m=1,\dots,M_{nj}} Z^{*}_{nj}(u_{nj}, m)^2 n / M_{nj}; j, n\}$ are }\DIFaddend uniformly integrable.
\end{lem}

\begin{proof}
  \DIFdelbegin \DIFdel{For }%DIFDELCMD < \eqref{eq:35}%%%
\DIFdel{, take $\epsilon > 0$. The sum
  }\[
  \DIFdel{\sum_{\tau=0}^{n-1} \max_{m=1,\dots,m'} Z_n(\tau, m)^2 / m'
  }\]
  %DIFAUXCMD
\DIFdel{is uniformly integrable if and only if it is uniformly $L_1$-bounded
  and there exists an $\epsilon'$ such that
  }\[
  \DIFdel{\sup_{n, m'} \E\Big( \ind(A) \sum_{\tau=0}^{n-1} \max_{m=1,\dots,m'} Z_n(\tau, m)^2 / m' \Big)
  < \epsilon
  }\]
  %DIFAUXCMD
\DIFdel{for every event $A \in \Omega_n$ satisfying $\pr(A) < \epsilon'$.
  (\mbox{%DIFAUXCMD
\citealp{Dav:94}}%DIFAUXCMD
, 12.9.)
  Lemma~\ref{L8} ensures uniform integrability of
  $\max_{m=1,\dots,m'} Z_n'(\tau, m)^2 n/m'$, so there exists an
  $\epsilon'$ such that
  }\begin{displaymath}
    \DIFdel{\sup_{n, m', \tau} \E\big( \ind(A) \max_{m=1,\dots,m'} Z_n'(\tau, m)^2 n / m' \big) \leq \epsilon / 9
  }\end{displaymath}
  %DIFAUXCMD
\DIFdel{for all $A \in \Omega_n$ with $\pr(A) \leq \epsilon'$. Then, writing
  }\begin{align*}
    \DIFdel{Z_n(\tau, m)}&\DIFdel{= Z_n'(\tau, m)
  ...
  %DIFAUXCMD
\DIFdel{gives the relationship
  }\begin{align*}
    \DIFdel{\E\Big(\ind(A) \sum_{\tau=0}^{n-1} }& \DIFdel{\max_{m=1,\dots,m'} Z_n(\tau, m)^2/m'\Big) }\\
    &\DIFdel{\leq 3 \sum_{\tau=0}^{n-1} \E\big( \ind(A)  \max_{m=1,\dots,m'} Z'_n(\tau, m)^2/m'\big) }\\
    &\DIFdel{\quad+(3/n m') \E\Big(\ind(A) \sum_{\tau=0}^{n-1} \max_{m=1,\dots,m'}
    \Big(\sum_{t \in I_n(\tau, m)} (\mu_{nt} - \bar \mu_n) \Big)^2\Big) }\\
    &\DIFdel{\quad+ (3 m' / n) \E (\ind(A) Z'_n(0, n)^2) }\\
    &\DIFdel{\leq (2 \epsilon/3) + (3 \epsilon' m' / n)\sum_{t=1}^n (\mu_{nt} - \bar \mu_n)^2
  }\end{align*}
  %DIFAUXCMD
\DIFdel{where the last inequality holds as a consequence of $A$'s
  ...
  similar, which completes the }\DIFdelend \DIFaddbegin \DIFadd{We will only present a proof of~}\eqref{eq:30}\DIFadd{. Uniform integrability
  ...
  follows an abbreviated version of the }\DIFaddend proof of~\DIFdelbegin \DIFdel{(\ref{eq:35})}\DIFdelend \DIFaddbegin \eqref{eq:30}\DIFaddend .

%DIFDELCMD <   %%%
\DIFdel{For~(\ref{eq:30}), we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend will first show that, for every $\epsilon > 0$ \DIFaddbegin \DIFadd{and large enough
  $n$}\DIFaddend , there exists an $\epsilon' > 0$ with the property that
  \begin{equation}\label{eq:31}
    \DIFdelbegin %DIFDELCMD < \Em %%%
\DIFdelend \DIFaddbegin \E \DIFaddend \Big(\ind(A) \times \max_{m = 1,\dots,m'} Z^*_n(\tau, m)^2 n / m'\Big) < \epsilon
  \end{equation}
  for any event \DIFdelbegin \DIFdel{$A \in \Fs_n \equiv \sigma(X_{n1},\dots,X_{nn};\,
  M_{n1},\dots,M_{n,J_n};\, J_n)$ }\DIFdelend \DIFaddbegin \DIFadd{$A \in \Fs_n \equiv \sigma(X_{n1},\dots,X_{nn})$ }\DIFaddend with
  $\pr(A) \leq \epsilon'$. Then we will show that this property
  implies uniform integrability.\footnote{%DIF > 
    The novelty in this part of the proof is the measurability
    requirement. The sigma-field $\Fs_n$ intentionally excludes the
    \DIFdelbegin\DIFdel{block start periods $u_{n1},\dots,u_{n,J_n}$}\DIFdelend
    \DIFaddbegin\DIFadd{random variables that define the bootstrap (block length and block
    start periods) and any $A \in \Fs_n$ is independent of those
    variables}\DIFaddend,
    so $Z_n^{*}(\tau, m)^2$ is not $\Fs_n$-measurable.
    \DIFdelbegin\DIFdel{If we instead proved~\eqref{eq:31}~for all $A \in \Omega_n$,
      as we did for the first part of the proof,}\DIFdelend\
    \DIFaddbegin\DIFadd{If $\Fs_n$ were enlarged to include those variables}\DIFaddend\
    then uniform integrability would be an immediate consequence
    \DIFaddbegin\DIFadd{of~\eqref{eq:31}}\DIFaddend\
    and would not need to be shown separately. See \citet[Theorem 12.9]{Dav:94}.}

  Take an arbitrary \DIFdelbegin \DIFdel{$\epsilon > 0$ }\DIFdelend \DIFaddbegin \DIFadd{$\delta > 0$ }\DIFaddend and a value of
  \DIFdelbegin \DIFdel{$\epsilon' > 0$} \DIFdelend
  \DIFaddbegin \DIFadd{$\delta' > 0$} \DIFaddend
  so that
  \DIFdelbegin\begin{displaymath}
    \DIFdel{\sup_{m'=1,\dots,n} \Em \Big(\ind(A) \sum_{\tau = 0}^{n-1} \tfrac{1}{m'} \max_{m = 1,\dots,m'}
    Z_n(\tau, m)^2\Big) < \epsilon / 6
  }\end{displaymath}
  %DIFAUXCMD
\DIFdelend
  \DIFaddbegin\begin{equation}\DIFadd{\label{eq:35}
    \sup_{m'=1,\dots,n} \E \Big(\ind(A) \max_{m = 1,\dots,m'}
    Z_n'(\tau, m)^2 n / m' \Big) < \delta
  }\end{equation}
  \DIFaddend for any $A \in \Fs_n$ with \DIFdelbegin \DIFdel{$\pr(A) \leq \epsilon'$, which is
  shown to exist in the first part of this Lemma. Then, for }\DIFdelend \DIFaddbegin \DIFadd{$\pr(A) \leq \delta'$; the existence
  of $\delta'$ is ensured by Lemma~\ref{L8}. For }\DIFaddend any $x$,
  define $J(x)$ to be the block index such that $K_{n,J(x) - 1} < x
  \leq K_{n,J(x)}$. For any $m$, we can decompose $Z_n^{*}(\tau, m)$
  \DIFdelbegin \DIFdel{as
  }\DIFdelend \DIFaddbegin \DIFadd{into a summation of conditionally independent blocks
  ...
  \begin{align}
  \DIFdelbegin
\E_{\DIFdel{\mathcal{M}}}\DIFdel{\Em^* }\DIFdelend \Big(&\ind(A) \times \max_{m = 1,\dots,m'} n \, Z^*_n(\tau, m)^2 / m'\Big)
  \notag \\
  &\leq 3\; \E_{\DIFdelbegin\DIFdel{\mathcal{M}}\DIFdelend} \DIFadd{\Big[}\DIFaddend
  \Em^*\Big[\ind(A) \tfrac{n}{m'}\max_{m = 1,\dots, m'}
  \Big(Z_n^*(\tau, K_{n,J(\tau)} - \tau) + \sum_{j=J(\tau)+1}^{J(\tau + m)-1} Z_{nj}^*\Big)^2\Big]
  \notag \\
  &\quad + 3 \; \E_{\DIFdelbegin\DIFdel{\mathcal{M}}\DIFdelend}
  \Em^* \big[\ind(A) \tfrac{n}{m'}
  \max_{m=1,\dots,m'} Z_n^*(K_{n,J(\tau+m)-1}, m - K_{n,J(\tau + m)-1})^2 \big]\DIFaddbegin \Big]\DIFaddend \label{eq:6}
  \end{align}
  almost surely.
...
\DIFaddbegin \begin{align*}
  \E_{\DIFdelbegin\DIFdel{\mathcal{M}}\DIFdelend} \Em^* & \Big[\ind(A) \tfrac{n}{m'}\max_{m = 1,\dots, m'}
  \Big(Z_n^*(\tau, K_{n,J(\tau)} - \tau)
  + \sum_{j=J(\tau)+1}^{J(\tau + m)-1} Z_{nj}^*\Big)^2\Big]\\
  &\leq \tfrac{1}{m'} \sum_{u = 0}^{n-1} \DIFadd{\E \Big[}\Em \big[
  \ind(A) Z_n(u, K_{n,J(\tau)} - \tau)^2\big] +
  \sum_{j=J(\tau)+1}^{J(\tau + m')-1} \Em \big[\ind(A) Z_n(u, M_{nj})^2]\DIFadd{\Big] }\\
  &\DIFadd{\leq \tfrac{3}{m'} \sum_{u = 0}^{n-1} \E\Big[\Em\big[
  \ind(A) Z'_n(u, K_{n,J(\tau)} - \tau)^2\big]
  + \tfrac{1}{n} \Pm[A] \Big(\sum_{t \in I_n(u, K_{n,J(\tau)} - \tau)}
  (\mu_{nt} - \bar \mu_n)\Big)^2 }\\
  & \DIFadd{\quad+ \sum_{j=J(\tau)+1}^{J(\tau + m')-1}
  \Em \big[\ind(A) Z_n'(u, M_{nj})^2]
  + \tfrac{1}{n} \Pm[A]
  \sum_{j=J(\tau)+1}^{J(\tau + m')-1}\Big( \sum_{t \in I_n(u,M_{nj})}
  (\mu_{nt} - \bar{\mu}_n) \Big)^2 }\\
  & \DIFadd{\quad+ \Em\big( \ind(A) Z_n'(u, n)^2 \big)
  \sum_{j=J(\tau)}^{J(\tau+m')-1} M_{nj}^2/n^2 \Big] }\\
  &\DIFadd{\leq (3\delta/m') \E (K_{n,J(\tau+m')-1} - \tau)
    + 3\delta \E\Big(\sum_{j=J(\tau)}^{J(\tau+m')-1} M_{nj}^2/n^2 \Big) }\\
  &\DIFadd{\quad+ \tfrac{3\delta'}{nm'} \sum_{u=0}^{n-1}
    \E\Big( \Big(\sum_{t \in I_n(u, K_{n,J(\tau)} - \tau)}
  (\mu_{nt} - \bar \mu_n)\Big)^2 +
  \sum_{j=J(\tau)+1}^{J(\tau + m')-1}\Big( \sum_{t \in I_n(u,M_{nj})}
  (\mu_{nt} - \bar{\mu}_n) \Big)^2 \Big) }\\
  & \DIFadd{< C_1 \delta + C_2 p_n \delta'
  }\end{align*}
\DIFdel{and this term is less than $\epsilon/6$ }\DIFdelend 
  \DIFadd{for some finite constants $C_1$ and $C_2$.
  The second inequality comes from writing
  }\begin{align*}
    \DIFadd{Z_n(\tau, m) }&\DIFadd{= Z_n'(\tau, m)
    + \tfrac{1}{\sqrt{n}} \sum_{t \in I_n(\tau, m)}
    \big((\mu_{nt} - \bar \mu_n) + (\bar \mu_n - \bar X_n)\big) }\\
    &\DIFadd{= Z_n'(\tau, m)
    + \tfrac{1}{\sqrt{n}} \sum_{t \in I_n(\tau, m)} (\mu_{nt} - \bar \mu_n)
  ...
  \DIFadd{the third follows }\DIFaddend by construction of $A$
  \DIFdelbegin \DIFdel{.
  }\DIFdelend \DIFaddbegin \DIFadd{and the fourth follows from an argument similar to Lemma~\ref{L3}.
}

  \DIFaddend For the second term on the \allcaps{RHS} \DIFaddbegin \DIFadd{of}\DIFaddend ~\eqref{eq:6}, use the
  sequence of inequalities
  \begin{align*}
  \DIFaddbegin \E \DIFaddend \Em\DIFdelbegin %DIFDELCMD < \Em%%%
\DIFdelend ^* \big[\ind(A) &\tfrac{n}{m'}
  \max_{m=1,\dots,m'} Z_n^*(K_{n,J(\tau+m)-1}, m - K_{n,J(\tau + m)-1})^2 \big] \\
  &\leq \DIFaddbegin \E \Big(\DIFaddend \sum_{j=J(\tau) + 1}^{J(\tau+m')-1} \tfrac{n}{m'} \Em\DIFdelbegin %DIFDELCMD < \Em%%%
\DIFdelend ^* \big[\ind(A)
  \max_{m=1,\dots, \min(M_{nj}, m' - K_{n,j-1})} Z_n^*(K_{n,j}, m)^2 \big]\DIFaddbegin \Big) \DIFaddend \\
  &\leq \DIFaddbegin \E \Big(\DIFaddend \sum_{j=J(\tau) + 1}^{J(\tau+m')} \DIFdelbegin %DIFDELCMD < \tfrac{M_{nj}}{m'} %%%
\DIFdelend \tfrac{\DIFdelbegin\DIFdel{M_{nj}}\DIFdelend\DIFaddbegin\DIFadd{1}\DIFaddend}{m'} \sum_{u=0}^{n-1} \Em \big[\ind(A)
  \max_{m=1,\dots,\min(M_{nj}, m' - K_{n,j-1})} Z_n(u, m)^2 \DIFdelbegin \DIFdel{/ M_{nj} }\DIFdelend \big]\DIFaddbegin \Big) \DIFaddend \\
  &\leq \DIFdelbegin\DIFdel{\sum_{j=J(\tau) + 1}^{J(\tau+m')} \epsilon \cdot \min(M_{nj}, m' - K_{n,j-1}) / 6 m'}\DIFdelend \\
  &\leq \DIFaddbegin\DIFadd{\E \Big(\sum_{j=J(\tau) + 1}^{J(\tau+m')} \tfrac{3}{m'} \sum_{u=0}^{n-1} \Big\{
  \Em \big[\ind(A)
  \max_{m=1,\dots,\min(M_{nj}, m' - K_{n,j-1})} Z_n'(u, m)^2 \big]}\DIFaddend \\
  &\quad\DIFaddbegin\DIFadd{+ \tfrac{1}{n} \Pm[A] \max_{m = 1,\dots,\min(M_{nj}, m'-K_{n,j-1})}
  \Big(\sum_{t \in I_n(u,m)} (\mu_{nt} - \bar{\mu}_n)\Big)^2}\DIFaddend \\
  &\quad\DIFaddbegin\DIFadd{+ \tfrac{\min(M_{nj},m'-K_{n,j-1})^2}{n^2} \Em( \ind(A) Z_n'(u, n)^2 ) \Big\}\Big)}\DIFaddend \\
  &\leq \DIFdelbegin \DIFdel{\epsilon/6.
  }\DIFdelend \DIFaddbegin \DIFadd{C'_1 \delta + C_2' \delta' p_n
  }\DIFaddend \end{align*}
  \DIFdelbegin \DIFdel{Together, these bounds imply~(\ref{eq:31})}\DIFdelend \DIFaddbegin \DIFadd{for constants $C_1'$ and $C_2'$. To express these bounds in the form
  of~}\eqref{eq:31}\DIFadd{, let $\epsilon > 0$ be arbitrary, choose $\delta'$
  such that~}\eqref{eq:35} \DIFadd{holds for $\delta = \epsilon/(2C_1+2C_1')$,
  and let $\epsilon' = \min(\delta', \epsilon/(2 C_2 + 2C_2'))$}\DIFaddend .
  ...
  \begin{equation*}
    \pr_{\DIFdelbegin \DIFdel{\mathcal{M}}\DIFdelend}
    \Big[ \max_{m = 1,\dots,m'} n \, Z^*_n(\tau, m)^2 / m' > C_0 \Big] \leq \epsilon'.
  \end{equation*}
  Markov's inequality guarantees the existence of this $C_0$. Then
  \begin{equation*}
    \E_{\DIFdelbegin\DIFdel{\mathcal{M}}\DIFdelend}
    \Big(\max_{m = 1,\dots,m'} n \, Z^*_n(\tau, m)^2 / m'
